...
| 성능 | 32~40 fps (32비트에서 약 40, 64비트에서 32.8 관측 — 원인 미특정) |
...
두 가지가 됩니다. 조종기가 꽂혀 있으면 버튼이 우선, 없으면 터치입니다.

- **터치** — 누른 x 위치로 캐릭터가 따라옵니다
- **아두이노 버튼 조종기** — 좌/우 버튼을 누르고 있는 동안 움직입니다. 꽂으면 자동 인식합니다 ([만드는 방법](arduino-controller/README.md))
...
게임을 물리 버튼으로 조작한다. 아두이노가 버튼을 읽어 **시리얼로 문자 하나**를 보낸다.

```
[◀ 버튼] D2 ─┐
[▶ 버튼] D3 ─┴→ 아두이노 UNO → USB 9600bps → 게임
                              'L' 'l' 'R' 'r'
```

배선은 버튼마다 두 가닥뿐 — **한쪽 다리를 D2(또는 D3), 다른쪽을 GND.** `INPUT_PULLUP` 이라
저항이 필요 없다. 누름(`L`/`R`)과 뗌(`l`/`r`)을 각각 보내므로 **누르고 있는 동안 이동**이 된다.

`poop-dodge.py` 가 `/dev/ttyACM*` → `/dev/ttyUSB*` 순으로 찾아 자동 연결하고, 없으면 터치로
간다. pygame 게임은 `pygame-on-lcd.py` 가 포트명을 바꿔 끼우므로 게임 코드를 안 고쳐도 된다.
...
```
batch#001 (5) BTN_TOUCH=1 ABS_X=3733 ABS_Y=1826 ABS_PRESSURE=54 SYN_REPORT=0
```

## 구현에서 신경 쓴 것

이 네 가지가 이 코드의 핵심이고, 전부 실제로 한 번씩 틀려본 뒤에 얻은 것입니다.
...
이벤트가 `BTN_TOUCH=1 → ABS_X → ABS_Y → ABS_PRESSURE → SYN` 순으로 오기 때문에, 압력으로 판정하면 `BTN_TOUCH`로 잡은 눌림 상태가 뒤따르는 압력값에 덮입니다. `BTN_TOUCH` 하나만 보면 임계값을 고를 일도 없습니다.

> 🔴 여기 원래 *"`xohms` 설정이 없으면 ADS7846은 압력을 항상 0으로 보고한다"* 고 써뒀는데, 같은 패널·같은 오버레이(`xohms` 없음)로 재측정하니 **`ABS_PRESSURE`가 14~90으로 정상 보고됐습니다**(38샘플 전부 비0). 그 진단은 재현되지 않습니다. 당시엔 아래 4번의 `struct` 파싱 버그가 남아 있었고 그 영향으로 보이지만, 그 환경이 없어 확정하지 못했습니다.
...
**3. 터치 범위는 커널이 알려주는 값을 믿을 수 없습니다**

`EVIOCGABS` 로 물어보면 `ABS_X`/`ABS_Y` 모두 `min=0 max=4095` 가 옵니다. 12비트 ADC의 전체 범위, 즉 **드라이버 기본값이고 이 패널의 실측값이 아닙니다.**

실제로 양 끝을 눌러보면 이렇습니다.

`touch-dump.py`로 화면 **물리 가장자리**를 눌러 재보면 이렇습니다.

| 누른 곳 | ABS_X |
|---|---|
| 왼쪽 가장자리 | 281 ~ 431 (중앙값 329) |
| 오른쪽 가장자리 | 3703 ~ 3742 (중앙값 3732) |

유효 폭이 `281~3742`, 전체의 **84.5%** 입니다. `0~4095`로 480픽셀에 매핑하면 왼쪽 끝이 33px, 오른쪽 끝이 439px로 계산되어 **화면 좌우 합쳐 약 15%에 닿지 않습니다.**

그래서 실측값을 기본으로 두고, **그보다 넓은 값이 들어오면 그때그때 넓힙니다.** 보정 절차도, 저장 파일도 없습니다. 다른 패널에서도 몇 번 누르면 맞아집니다.

```python
RAW_LO, RAW_HI = 281, 3742      # 이 패널 실측값

if value < self.lo:             # 관측되는 대로 넓어진다
    self.lo = value
elif value > self.hi:
    self.hi = value
```

화면 가로축이 `ABS_Y` 인 패널이라면 오버레이에 `swapxy` 를 주면 됩니다 — 코드는 그대로 씁니다.

**4. `struct input_event` 크기는 userland 비트수를 따릅니다**
